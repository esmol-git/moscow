		...
		plugins: [
			...
			...(isProduction && templateConfig.server.copyfiles ? [templateImports.viteStaticCopy({
				targets: [
					...
					// PHP sendmail (dist/php/sendmail/) — для dev/альтернативного пути
					{
						src: 'src/php',
						dest: 'php',
						structured: true,
					},
					// PHP sendmail в dist/sendmail/ — форма шлёт на sendmail/index.php, при деплое этот путь обновляется
					{
						src: 'src/php/sendmail',
						dest: 'sendmail',
						structured: true,
					},
				],
			...
			// Гарантированное копирование sendmail в dist/sendmail (для деплоя на хостинг)
			...(isProduction && templateConfig.server.copyfiles ? [{
				name: 'copy-sendmail-to-dist',
				apply: 'build',
				enforce: 'post',
				closeBundle: {
					order: 'pre',
					handler: async () => {
						const srcDir = path.join(process.cwd(), 'src/php/sendmail');
						const destDir = path.join(process.cwd(), 'dist', 'sendmail');
