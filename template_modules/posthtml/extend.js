...
function handleExtendsNodes(tree, options, messages) {
	match.call(tree = applyPluginsToTree(tree, options.plugins), { tag: options.tagName }, extendsNode => {
		...
		let layoutPath = path.resolve(options.root, extendsNode.attrs.src);
		if (!fs.existsSync(layoutPath)) {
			const fallbackRoot = path.join(process.cwd(), 'src');
			const fallbackPath = path.join(fallbackRoot, extendsNode.attrs.src);
			if (fs.existsSync(fallbackPath)) layoutPath = fallbackPath;
		}
